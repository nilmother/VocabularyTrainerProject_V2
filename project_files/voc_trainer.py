...
class App:
    ...
    def __init__(self, root):
        ...
        self.lbl_stats.place(y=550, x=10)
    ...
    def change_unit(self, event=None):
        try:
            ...
            current_word = util.create_exercise(self.filtered_df, self.task_name)
            self.question_line.set(current_word[0])
    ...
    def show_exercise(self, filtered_df, task_name):
            # Working on the exercise logic
            current_word = util.create_exercise(self.filtered_df, task_name)
            # self.question_line.set(current_word[0])
    ...
    def load_task(self, task_name):
        try:
            ...
            self.df = util.load_data(self.task_name)
            ...
            self.show_exercise(self.filtered_df, task_name)
            
            # Update the stat_line based on the filtered DataFrame
            self.info_line.set(f"Now learning {self.task_name}.")      
            self.stat_line.set(f"Unit {self.currentUnit or 'ALL'}: {len(self.filtered_df)} vocables available.")
